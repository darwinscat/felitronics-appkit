...
//==============================================================================
// The build stamp for a window's bottom bar — "v0.6.0 · Standalone" — and a door to the About
// window. What version is running, and in what wrapper, is a fact about the RUN, and the bottom bar
// is where a product keeps those: the mode, the latency, the sample rate, the load. Two products in
// this family had written this line separately before it moved here.
//
// It paints only text: no tile, no frame — the line lifts out of the background under the cursor,
// which is the only thing that says it can be pressed. It owns no window either: the About dialog
// belongs to VersionBadge, so this takes an `onClick` rather than a badge, and a product is free to
// point it at something else entirely.
//
// Colours and type come from the product (see setColours / setDisplayFont). The defaults are a
// neutral dim white — legible on a dark bar, and nobody's brand.
...
public:
    ...
    // window spells it.
    VersionStamp (UpdateChecker& c, juce::String format)
        : checker (c)
    {
        ...
        const bool clean = update::isCleanRelease (describe.toStdString());
    ...
    // The product's own ink. `rest` is the line at rest, `hover` the lift under the cursor, `dot`
    // the "a newer release is known" mark — a product's attention colour, whatever that is for it.
    void setColours (juce::Colour rest, juce::Colour hover, juce::Colour dot)
    {
        ink = rest; inkHover = hover; dotInk = dot;
        repaint();
    }
    ...
    void paint (juce::Graphics& g) override
    {
        ...
        if (updateDot)
        {
            ...
            g.setColour (dotInk);
            g.fillEllipse ((float) slot.getRight() - 9.0f, (float) slot.getCentreY() - 3.0f, 6.0f, 6.0f);
        }

        g.setColour (isMouseOver() ? inkHover : ink);
        chrome::drawTracked (g, text, r.toFloat(), font(), tracking, juce::Justification::right);
    }
...
private:
    ...
    juce::Colour ink      { 0x6bffffff };   // a dim white: legible on a dark bar, nobody's brand
    juce::Colour inkHover { 0xffd8d8d8 };
    juce::Colour dotInk   { 0xffff8a3d };   // the family's attention orange, until a product says otherwise
