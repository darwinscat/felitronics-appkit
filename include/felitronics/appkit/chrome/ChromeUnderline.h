...
// fills with), flat again. It is a top-most, MOUSE-TRANSPARENT overlay, so the border is one uniform
// hairline that just happens to dip, with no line-junction to mismatch in thickness/position.
//
// It reads the blister's live bounds through getLocalArea (so it never assumes a shared origin) and
// SELF-SUBSCRIBES as a ComponentListener on the blister — so it repaints itself whenever the blister
// moves or resizes, with NO manual wiring for the consumer to forget. The blister is always laid out
// at its full preferred width — even at negative x — so the left cut is produced purely by the WINDOW
// clip; the overlay follows the blister off-screen without intersecting the viewport.
//
// The blister pointer is cleared foolproof-ly if the blister is destroyed first (componentBeingDeleted),
// so a mis-ordered teardown paints nothing rather than dangling. Metrics + theme are held BY VALUE.
class ChromeUnderline final : public juce::Component,
                              private juce::ComponentListener
{
public:
    ChromeUnderline (BrandBlister& b, const ChromeMetrics& m, const ChromeTheme& t)
        : blister (&b), metrics (m), theme (t)
    {
        setInterceptsMouseClicks (false, false);
        blister->addComponentListener (this);   // repaint on any blister move/resize — no manual wiring
    }

    ~ChromeUnderline() override
    {
        if (blister != nullptr)
            blister->removeComponentListener (this);
    }
...
private:
    void componentMovedOrResized (juce::Component&, bool, bool) override { repaint(); }
    void componentBeingDeleted   (juce::Component& c) override { if (&c == blister) blister = nullptr; }

    BrandBlister*        blister;   // cleared if the blister outlived-check fails (componentBeingDeleted)
