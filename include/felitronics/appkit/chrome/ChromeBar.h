...
//   • RightPinned — a fixed run pinned to the right edge (laid out right-to-left); its left edge
//     defines the free band the RigidCenter centres in.
...
// explicit `height` for an overhanging cell and `hInset/vInset` for pinned glyph buttons).
...
struct ChromeBar
{
    ...
    int layout (juce::Rectangle<int> bar, int barHeight, int minStartX) const
    {
        ...
        // blister butts the edge). Per-item gapBefore is unused in this region.
        ...
        if (nItems > 0)
        {
            ...
            // compress (can go below minGap) instead of the block hanging over the right-pinned run.
            const int blockW   = juce::jmin (juce::jlimit (minW, maxW, avail - 2 * edgeMargin), avail - rightGuard);
            // Clamp the even gap at 0: a very narrow window (avail < sumFixed) drives blockW below
            // sumFixed, so the raw gap would go NEGATIVE and pull cells back over each other. A
            // product with a fixed min-window may never reach this branch (then it is a no-op and
            // pixel-identical); the clamp is correctness for a library consumer with no min-window.
            const int gap      = nItems > 1 ? juce::jmax (0, (blockW - sumFixed) / nGaps) : 0;
            ...
            // If a RightPinned run exists, keep the rigid block from crossing into its corner even when
            // the band is too narrow to hold the cells at all (actualW > avail): clip further off the
            // LEFT (the blister's designed off-screen slide) rather than overrunning the pinned buttons.
            // No-op when the band has room (the centred x already sits left of this cap), so this is
            // pixel-identical for a product with a real min-window; correctness only for a narrow library consumer.
            if (hasRightPinned)
                x = juce::jmin (x, bandRight - rightGuard - actualW);
            bool first = true;
            for (const auto& c : cells)
            {
