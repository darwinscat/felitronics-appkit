...
# Contributing to felitronics-appkit
...
**Why.** felitronics-appkit is AGPL-3.0-or-later and the copyright is held solely by its authors,
Oleh Tsymaienko and Alisa Lafoks (Darwin's Cat). A merged contribution leaves its author holding
...
its sibling in the family — core is the JUCE-free DSP, this is the JUCE side.
...
It takes away none of the freedoms the AGPL grants you. You may use felitronics-appkit, study it,
...
Third-party components keep their own licences and are unaffected by the policy above. This repo is
header-only, so most of what a consumer links it fetches itself — JUCE above all, which the consumer
supplies — but two OFL fonts are **compiled into these headers** and therefore travel inside every
binary built against them, and `felitronics::appkit_webp` fetches libwebp when explicitly enabled.
All of it, with the reasoning the OFL requires, is recorded in
[`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md). New dependencies must be AGPL-compatible
(BSD / MIT / Apache-2.0 / MPL-2.0) and are added to that file.
